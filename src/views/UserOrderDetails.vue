<template>
    <div>
        ...
        >
            <div class="hidden lg:flex lg:text-base lg:font-semibold lg:p-2">
                Your Order Details
            </div>
            <div class="bg-white p-2 my-2 tracking-wide text-sm">
                <p class="font-semibold">Order N°: {{ order.data.orderId }}</p>
                <p class="text-gray-800">Created At: {{ order.data.date }}</p>
                <p class="text-gray-800">Total: {{ order.data.total }} TND</p>
            </div>
            <div>
                    ...
                    >
                        <div class="text-sm space-y-2 border-b">
                            <span
                                ...
                                :class="
                                    order.data.deliveryStatus === 'pending'
                                        ? 'bg-red-400'
                                        : 'bg-green-500'
                                "
                                >{{ order.data.deliveryStatus }}</span
                            >
                            <p>08/01/2020</p>
                        </div>
                        <div class="flex p-2">
            ...
            <div>
                ...
                <div class="mx-2 p-4 bg-white rounded-md">
                    <div class="space-y-1 border-b">
                        ...
                        >
                        <p class="text-xs text-gray-500">Payment on delevery</p>
                    </div>
                </div>
            </div>
        </div>
...
export default {
    ...
    setup() {
        ...
        const order = ref(
            computed(() => {
                if (store.state.order.order === null) {
                    return {
                        orderId: '',
                        data: {
                            date: '',
        ...
        onMounted(() => {
            store.dispatch('order/getOrderById', route.query.orderId)
        })
        return { order }
    },
}
